...
export const GALLERY_INTRO = {
  ...
  caveat:
    "The two FREEZES (USDC, Oct-10) are the strongest result: the contract halts on its own re-derived divergence, no score needed. The two non-freeze catches show solvency-vs-liquidity discrimination — Aug is LIQUIDITY-led (borrow cap floors, max LTV barely moves), Feb is SOLVENCY-led at onset (max LTV floors first, borrow cap holds at baseline) — same model, opposite knobs. Feb is also where the ML earns its place (see the case). Cetus is incidental and out-of-scope — a SUI-native DEX bug, not the oracle/price-anomaly class — that the model still trips. Replayed scores are regression-verified identical to the validated reports. One substitution to be upfront about: live, divergence is Pyth oracle vs DeepBook CLOB mid, re-derived on-chain — but historical on-chain order-book data isn't archived, so these replays feed the detector the closest free analog instead: perp last vs index price (the crypto crashes), or spot vs the $1 peg (USDC). It's the same estimator on the same kind of price-vs-reference gap, not the live oracle↔CLOB signal itself; the freeze line shown is that proxy crossing 5%. Order-book depth (imbalance, spread) has no free history and isn't plotted here.",
};
