...
export const COPY: Record<string, CaseCopy> = {
  ...
  cetus: {
    ...
    read: "An incidental, out-of-scope catch on a SUI-native event: a single-asset contract-bug drain isn't the oracle/price-anomaly class Seawall is built for, yet the cross-venue volatility-velocity and divergence still trip the knobs. Treat it as evidence the model isn't blind to on-chain stress — not as a driver-discrimination demo (for that see Aug, liquidity-led, and Feb, solvency-led).",
    caveat:
...
export const GALLERY_INTRO = {
  ...
  caveat:
    "Honest scope: the two FREEZES (USDC, Oct-10) are the strongest result — the contract halts on its own re-derived divergence, no score needed. The solvency-vs-liquidity discrimination shows in the two non-freeze catches: Aug is LIQUIDITY-led (borrow cap leads to its floor, max LTV barely moves) and Feb is SOLVENCY-led at onset (max LTV floors first while borrow cap holds at baseline) — same model, opposite knobs. Feb also shows the ML adding value (it floors the knobs while divergence barely grazes the caution line) but reacting at the crisis, not predicting it. Cetus is an incidental, out-of-scope catch — a SUI-native DEX bug, not the oracle/price-anomaly class — that the model still trips. The replayed scores are regression-verified identical to the validated reports. Backtests use free CEX price/divergence history (last-vs-index, or vs the $1 peg) as the oracle↔market proxy; the live system re-derives the same shape from Pyth↔DeepBook on-chain. Public order-book DEPTH isn't archived, so depth features are shown live, not backtested.",
};
