...
export const COPY: Record<string, CaseCopy> = {
  feb2025: {
    ...
    read:
      "This is the guardian reacting to a sustained selloff, not predicting it — and it's the strongest evidence the ML adds something: the contract's own divergence never crosses caution, so without the score the knobs never move. The trade-off is timing — enforcement lands at the crisis, not 5 h early. For a clean predictive lead see USDC; for the in-block freeze see Oct-10.",
  },
  usdc2023: {
    detection:
  ...
  cetus: {
    ...
    caveat:
      "Honest boundary: the exploit lived on the Cetus DEX, not the CEX basis Seawall watches (divergence peaks just ~19 bps), so the response is modest and coincident — not predictive. Seawall covers the oracle/price-anomaly class; it is not a detector for a DEX logic-bug drain. Note the irony: Sui validators froze the hacker (③) by social consensus — the heavy-handed manual version of what Seawall does narrowly, automatically, and only toward safer.",
  },
};
