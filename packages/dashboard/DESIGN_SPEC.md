...
- **Gauge bands == on-chain constants** — RiskGauge keeps reading `BANDS` (`SCORE_LO 55 / SCORE_HI 80 / ALERT_SCORE 99`); the sparkline etch lines read `BANDS`; the corridor reads `MAX_LTV_BPS`/`BORROW_CAP_BPS`. The alias block (`--teal:var(--cyan)` etc.) means no JSX color reference changes.
