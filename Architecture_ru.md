...
Каждые 10 минут разжимаем настройки на одну ступень — примерно 10% диапазона каждого коридора — но только пока собственное свежее показание контракта остаётся чистым (отсутствие агента никогда не вызывает разжимание; это был бы fail-open). Полностью ужатый параметр возвращается к baseline примерно за десять шагов (~100 минут).

**Это медленное разжимание — ещё и окно для DAO.** Поскольку разжимание идёт постепенно и каждый шаг публикуется как событие, у DAO, следящего за чейном, есть время вмешаться до того, как коридор снова полностью откроется — мгновенно зажать рамки через `governance_set_corridor`, сменить агента или (после заморозки) контролировать разблокировку. Автоматически там, где это безопасно; вручную там, где это важно.
